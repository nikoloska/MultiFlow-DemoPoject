...
engine.onIntent(({ intent, trigger, voicePayload, colorPayload }) => {
  ...
  switch (intent) {
    case "FILTER_CUISINE":
      window.showToast?.('🍽️ Filtering ' + (CUISINE_MAP[voicePayload?.command] || '') + ' recipes', 'success'); {
    ...
    case "OPEN_RECIPE":
      window.showToast?.('📖 Opening recipe!', 'success'); {
    ...
    case "PREV_STEP":
      window.showToast?.('👈 Previous step'); {
    ...
    case "STOP":
      window.showToast?.('🔄 Showing all recipes'); {
