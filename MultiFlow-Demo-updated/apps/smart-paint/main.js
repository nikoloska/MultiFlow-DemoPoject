...
gesture.onData((event) => {
  if (event.type === "position") {
    // Always show finger cursor so user knows hand is tracked
    window.updateFingerCursor?.(event.payload.x, event.payload.y, isPainting);
    if (isPainting) {
...
engine.onIntent(({ intent, ...args }) => {
  switch (intent) {
    ...
    case "activateDraw":
      ...
      window.showToast?.("🎨 Drawing! Move your hand to paint", "success");
      break;
    ...
    case "stopDraw":
      ...
      window.showToast?.('⏸ Stopped. Say "paint" to resume.');
      break;
    ...
    case "setBackground":
      ...
      window.showToast?.("🎨 Background set to " + args.color + "!", "success");
      break;
...
function _updateLockProgress() {
  ...
  if (progress >= 1 && locked) {
    const newColor = locked.rgb.r + "," + locked.rgb.g + "," + locked.rgb.b;
    if (ring._lastLocked !== newColor) {
      ring._lastLocked = newColor;
      window.triggerLockPop?.();
    }
    ring.style.stroke = `rgb(${locked.rgb.r},${locked.rgb.g},${locked.rgb.b})`;
    _updateLockedColorIndicator(locked);
  } else {
    ring._lastLocked = null;
