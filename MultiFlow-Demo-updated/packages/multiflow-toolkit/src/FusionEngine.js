  ...
  for (const rule of rules) {
    ...
    if (match.gesture) {
      ...
      if (gestureMatch.direction) {
        ...
        // Detect swipe direction from movement over last 800ms of gesture events
        const recentGestures = buffer
          .filter(e => e.source === "gesture" && e.type === "position" && now - e.timestamp < 800)
          .sort((a, b) => a.timestamp - b.timestamp);

        let dir = "center";
        if (recentGestures.length >= 2) {
          const first = recentGestures[0].payload.x;
          const last  = recentGestures[recentGestures.length - 1].payload.x;
          const delta = last - first;
          if (delta > 0.12) dir = "right";       // moved right by >12% of frame
          else if (delta < -0.12) dir = "left";  // moved left by >12% of frame
          else dir = "center";
        } else {
          // Fallback: absolute position
          const x = gestureEvt.payload.x ?? 0.5;
          dir = x > 0.72 ? "right" : x < 0.28 ? "left" : "center";
        }
